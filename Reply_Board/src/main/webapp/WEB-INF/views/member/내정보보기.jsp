		...
		<div class="field" hidden="hidden">
			<label for="id">아이디<input type="text" value="${member.id}"
				...
				type="text" value="${member.name}" readonly="readonly" /></label> 
				 <form action="profile" enctype="multipart/form-data" method="post"><label
				for="profile">프로필사진<u:hasProfile>
					<img src="data:image/png;base64,${profile}" width="auto"
						height="auto" />
					<br>
				</u:hasProfile><u:nullProfile><img id="profile"
					src="/static/img/no_image.jpeg" width="100px" height="auto" /></u:nullProfile><input type="file" name="profileFile" id="profileFile" />
			</label><br><input type="submit" value="사진 변경하기"> </form><form action="email" method="post"><label for="email">이메일<input type="text"
				value="${member.email}" name="email"/><input type="submit" value="이메일 변경"></label></form> <a class="btn"
