...
public interface IMemberMapper {
	...
	@Select("SELECT * FROM member WHERE id=#{id} and password='MD5(#{password,jdbcType=VARCHAR}'")
	...
	Member 로그인하다(@Param("id") String id,@Param("password") String password);
	...
	@Insert("INSERT INTO member(id, name, password, email) VALUES(#{id},#{name},MD5(#{password}),#{email})")
	int 회원가입하다(Member member);

	@Update("UPDATE member SET password=MD5(#{password}) where id=#{id}")
